...
const authController = {
  ...
  async postSignup(req, res) {
    try {
      ...
      if (!isAdminEmail && !User.isValidCollegeEmail(email)) {
        req.flash('error', 'Only verified college students can access Campus OLX. Use your college email (@krct.ac.in, @krce.ac.in, @mkce.ac.in).');
        return res.redirect('/auth/signup');
      }
  ...
  async postLogin(req, res) {
    try {
      ...
      const isAdminEmail = email === 'jawaharm2007@gmail.com';
      if (!isAdminEmail && !User.isValidCollegeEmail(email)) {
        req.flash('error', 'Only verified college students can access Campus OLX.');
        return res.redirect('/auth/login');
      }
